...
class TestStdoutMonitorOutput:
    ...
    async def test_write_bytes(self) -> None:
        ...
        mock_stdout.write = write_side_effect  # type: ignore[method-assign]

        with patch.object(sys, "stdout", mock_stdout):
            await output.write(b'metric_value{label="test"} 42')

        # Combine buffer writes and stdout writes
    ...
    async def test_write_flushes_stream(self) -> None:
        """Test that stdout is flushed after writing."""
        output = StdoutMonitorOutput()
        mock_stdout = MagicMock()
...
class TestStderrMonitorOutputFromStdout:
    """Tests for StderrMonitorOutput class in stdout.py module."""
